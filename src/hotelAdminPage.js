...
function admin() {
  ...
  const handleAddRow = () => {
    ...
    const admin = document.getElementById("facility-manager").value;
    ...
    No
      ...
      <div className="w-2/5 bg-[#f8f9fa] relative">
        ...
        <div className="flex items-center absolute inset-0 mx-2.5 my-auto border border-solid border-gray-300 rounded-lg p-4 h-[3rem] cursor-pointer mt-[9rem]">
          <p className="text-[#313e4d]">Reviews</p>
        </div>
        <div>
  ...
  <div className="flex items-center mx-3.5 my-2.5">
    <p className="text-2xl font-normal text-[#313e4d] mr-3.5">Facilities</p>
        ...
        <tbody className="text-gray-700 text-xs">
          {currentRows.map((row, index) => (
            <tr key={index}>
              ...
              <td className="py-3 px-4">{row.facilitymanagerid}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
<div class="admininputbox">
  <form className="flex flex-col mt-2">
    <div className="flex flex-wrap justify-between">
      ...
      <label for="facility-manager" class="admininput-hotel">
      Facility Manager:
      </label>
      <input
        type="text"
